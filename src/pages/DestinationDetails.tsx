...
export default function DestinationDetails() {
  ...
  return (
    <div className="min-h-screen bg-white">
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2">
            <div className="relative rounded-2xl sm:rounded-3xl overflow-hidden shadow-2xl" onMouseEnter={() => setIsHovering(true)} onMouseLeave={() => setIsHovering(false)}>
                  ...
                  {gallery.map((src, idx) => (
                    <CarouselItem key={idx} className="basis-full">
                      <div className="relative">
                        <img src={src} alt={`${destination.name} ${idx + 1}`} className="w-full h-[280px] sm:h-[400px] md:h-[480px] lg:h-[520px] object-cover" />
              ...
              <div className="absolute bottom-4 sm:bottom-6 left-4 sm:left-6 right-4 sm:right-6 flex flex-col sm:flex-row items-start sm:items-end justify-between gap-3 pointer-events-none">
                <h1 className="text-white text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-light drop-shadow-lg" style={{ fontFamily: '"Instrument Serif", serif' }}>{destination.name}</h1>
            ...
            <div className="mt-4 overflow-x-auto pb-2 -mx-4 sm:mx-0 px-4 sm:px-0 scrollbar-hide">
                  ...
                  >
                    <div className="relative rounded-md sm:rounded-lg overflow-hidden">
                      <img
                        ...
                        className="w-20 h-16 sm:w-24 sm:h-20 md:w-28 md:h-22 object-cover"
                      />
                    </div>
                  </button>
                ))}
          ...
          <aside className="lg:sticky lg:top-24 lg:self-start">
            <div className="rounded-2xl sm:rounded-3xl border border-slate-200 shadow-xl p-5 sm:p-6 bg-white">
              <div className="space-y-2 sm:space-y-3">
                <div className="text-slate-500 text-sm sm:text-base">Starting from</div>
                <div className="text-3xl sm:text-4xl font-bold text-teal-700" style={{ fontFamily: 'Manrope, sans-serif' }}>{priceFormatted}</div>
      ...
      <section className="px-4 sm:px-6 py-12 sm:py-16 bg-gradient-to-br from-slate-50 via-blue-50 to-teal-50">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12">
          <div className="lg:col-span-2 space-y-6 sm:space-y-8">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-light text-slate-800" style={{ fontFamily: '"Instrument Serif", serif' }}>Overview</h2>
            <p className="text-base sm:text-lg leading-relaxed text-slate-700" style={{ fontFamily: 'Manrope, sans-serif' }}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-10">
              <div>
                <h3 className="text-lg sm:text-xl font-semibold mb-4 text-slate-800">What's Included</h3>
                <ul className="space-y-3">
                  {included.map((it, idx) => (
                    <li key={idx} className="flex items-start gap-3 text-sm sm:text-base text-slate-700">
                      <it.icon className="w-5 h-5 flex-shrink-0 text-teal-600 mt-0.5" />
                      <span>{it.label}</span>
                    </li>
                  ))}
                </ul>
              ...
              <div>
                <h3 className="text-lg sm:text-xl font-semibold mb-4 text-slate-800">What's Not Included</h3>
                <ul className="space-y-3">
                  {excluded.map((label, idx) => (
                    <li key={idx} className="flex items-start gap-3 text-sm sm:text-base text-slate-700">
                      <XIcon className="w-5 h-5 flex-shrink-0 text-slate-400 mt-0.5" />
                      <span>{label}</span>
                    </li>
                  ))}
                </ul>
            ...
            <div className="mt-6">
              <h3 className="text-lg sm:text-xl font-semibold mb-4 text-slate-800">Signature Itinerary</h3>
              <ol className="relative border-l-2 border-teal-100 pl-5 sm:pl-6 space-y-6">
                {itinerary.map((d) => (
                  <li key={d.day} className="relative">
                    <span className="absolute -left-[1.6rem] top-1 w-2.5 h-2.5 rounded-full bg-teal-500" />
                    <div className="font-semibold text-sm sm:text-base text-slate-800">Day {d.day}: {d.title}</div>
                    <div className="text-sm sm:text-base text-slate-600 mt-1">{d.desc}</div>
                  </li>
                ))}
              </ol>
            </div>
          ...
          <div>
            <h3 className="text-lg sm:text-xl font-semibold mb-4 text-slate-800">Upcoming Departures</h3>
            <div className="space-y-3 sm:space-y-4">
              {batches.map((b) => (
                <div key={b.id} className="rounded-xl sm:rounded-2xl border border-slate-200 bg-white p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-sm">
                  <div>
                    <div className="font-semibold text-sm sm:text-base text-slate-800">{b.date}</div>
                    <div className="text-xs sm:text-sm text-slate-500 flex items-center gap-3 mt-1">
                  ...
                  <button className={cn('px-4 py-2 rounded-full text-sm font-semibold transition', b.available > 0 ? 'bg-teal-600 text-white hover:bg-teal-700' : 'bg-slate-200 text-slate-600')}
      ...
      <section className="px-4 sm:px-6 py-12 sm:py-16">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-light text-slate-800 mb-8 sm:mb-10" style={{ fontFamily: '"Instrument Serif", serif' }}>Tailored Packages</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            {packages.map((p, idx) => (
              <div key={idx} className="rounded-2xl sm:rounded-3xl border border-slate-200 bg-white p-5 sm:p-6 shadow-lg hover:shadow-xl transition">
                <div className="text-xs sm:text-sm text-slate-500">For up to {p.people} {p.people === 1 ? 'guest' : 'guests'}</div>
                <div className="text-base sm:text-lg md:text-xl font-semibold text-slate-800 mt-2">{p.name}</div>
                <div className="text-teal-700 text-lg sm:text-xl md:text-2xl font-bold mt-3 sm:mt-4">{new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 }).format(p.price)}</div>
                <ul className="mt-3 sm:mt-4 space-y-2 text-xs sm:text-sm text-slate-600">
                  {p.perks.map((perk, i) => (
                    <li key={i} className="flex items-start gap-2">
                      <Check className="w-4 h-4 flex-shrink-0 text-teal-600 mt-0.5" />
                      <span>{perk}</span>
                    </li>
                  ))}
                </ul>
                <button className="mt-5 sm:mt-6 w-full px-5 py-2.5 sm:py-3 rounded-full bg-teal-600 text-white text-sm sm:text-base font-semibold hover:bg-teal-700 transition">Enquire</button>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="px-4 sm:px-6 py-12 sm:py-16 pb-16 sm:pb-20 bg-gradient-to-br from-teal-50 via-blue-50 to-slate-50">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-light text-slate-800 mb-8 sm:mb-10" style={{ fontFamily: '"Instrument Serif", serif' }}>Guest Stories</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
            {(details?.reviews || [
              ...
              <div key={idx} className="group relative bg-white rounded-2xl sm:rounded-3xl p-5 sm:p-6 md:p-8 shadow-lg hover:shadow-xl transition-all">
                ...
                <p className="text-sm sm:text-base text-slate-600 leading-relaxed mb-4 sm:mb-6" style={{ fontFamily: 'Manrope, sans-serif' }}>
                  "{t.text}"
                </p>
                <div className="text-xs sm:text-sm text-slate-500" style={{ fontFamily: 'Manrope, sans-serif' }}>
                  {t.name} • {t.origin}
                </div>
              </div>
            ))}
